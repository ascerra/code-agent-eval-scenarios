...
description: >-
  ...
  (GitHub, GitLab, Jira, Linear). Emphasizes test-first development,
  explicit reasoning at decision points, self-review before commit,
  and minimal diff discipline.
---
...
- `github.com/*/issues` → use `gh issue view`
- `gitlab` with `/issues/` → use `glab issue view`
- `atlassian.net` or `jira` → use `jira issue view`
- If only `ISSUE_NUMBER` is set with no URL, default to the code host's
...
Fetch the issue using the detected issue tracker CLI:
...
**Exit: you know both platform CLIs (code host + issue tracker), and you
have the issue number, title, body, and labels loaded.**
...
If the issue references other issues or PRs/MRs, fetch them using the
appropriate CLI from step 1:
...
# GitLab
glab issue view <related-number>
glab mr view <related-number>

# Jira
jira issue view <related-key>
```
...
4. **Study recent commit messages.** Run `git log --oneline -10` to learn
...
- **Target branch** (from `TARGET_BRANCH` env var or `git rev-parse --abbrev-ref origin/HEAD`)
...
- Summarize the change in a way that a reviewer understands without reading
  ...
  contribution.
- Reference the issue using the platform's closing keyword:
  `Closes #<number>` (GitHub/GitLab), or the Jira/Linear ticket key.
