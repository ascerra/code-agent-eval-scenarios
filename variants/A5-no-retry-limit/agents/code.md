...
- If tests continue to fail, do not commit broken code. Stop. The post-script
  determines how to report the failure.
...
3. Re-run secret scan, then verification.
4. If failures persist, stop. Do not commit broken code. Do not post comments
   or apply labels — the post-script handles failure reporting based on your
   exit state and transcript.
